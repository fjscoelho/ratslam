 ...
 * openRatSLAM
 *
 * experience_map - Experience map implementation for RatSLAM.
 *
 * Copyright (C) 2012
...
// This file implements the ExperienceMap class for RatSLAM.
...
{
  return sqrt(
    (double)((exp1->x_m - exp2->x_m) * (exp1->x_m - exp2->x_m) + (exp1->y_m - exp2->y_m) *
    (exp1->y_m - exp2->y_m)));
}
...
bool ExperienceMap::calculate_path_to_goal(double time_s)
{
  ...
  if (
    exp_euclidean_m(&experiences[current_exp_id], &experiences[goal_list[0]]) < 0.1 ||
...
double ExperienceMap::get_subgoal_m() const
{
  return waypoint_exp_id == -1 ?
         ...
         sqrt(
    (double)pow(
      (experiences[waypoint_exp_id].x_m - experiences[current_exp_id].x_m), 2) +
    (double)pow(
      (experiences[waypoint_exp_id].y_m - experiences[current_exp_id].y_m), 2));
}
...
double ExperienceMap::get_subgoal_rad() const
{
  if (waypoint_exp_id == -1) {
    ...
    double curr_goal_rad = atan2(
      (double)(experiences[waypoint_exp_id].y_m - experiences[current_exp_id].y_m),
