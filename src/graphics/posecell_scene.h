...
public:
    PosecellScene(
      PosecellNetwork * in_pc, const std::string & media_path_in, const std::string & image_file_in)
      ...
      image_file(image_file_in)
    {
      ...
      device = irr::createDevice(
        irr::video::EDT_OPENGL, irr::core::dimension2d < irr::u32 > (window_width, window_height),
        32,
        false, false, false);
      ...
      for (int i = 0; i < NUM_PARTICLES; i++) {
        ...
        particles[i]->setMaterialTexture(
          0,
          pose_cells_scene->getVideoDriver()->getTexture((media_path + "/particle.bmp").c_str()));
      }
      ...
      proj.buildProjectionMatrixOrthoLH(
        (irr::f32)posecells->PC_DIM_XY * 2 + 1, (irr::f32)50, (irr::f32)0.01, (irr::f32)100);
    ...
    void update_position(double x, double y, double th, int PC_DIM_XY, int PC_DIM_TH)
    {
      position_line->setPosition(
        irr::core::vector3df(
          (irr::f32)(x - (double)PC_DIM_XY / 2), (irr::f32)(th - (double)PC_DIM_TH / 2),
      ...
      pose_cell_history->addPoint(
        irr::core::vector3df(
          (irr::f32)(x - (double)PC_DIM_XY / 2), (irr::f32)(-(double)PC_DIM_TH / 2),
          (irr::f32)(y - (double)PC_DIM_XY / 2)));
    }
    ...
    void update_scene()
    {
      ...
      update_position(
        posecells->best_x, posecells->best_y, posecells->best_th, posecells->PC_DIM_XY,
        posecells->PC_DIM_TH);
    }
