...
public:
    ExperienceMapScene(
      ExperienceMap * in_map, const std::string & media_path_in, const std::string & image_file_in)
      : exp_map_scene(NULL),
      exp_map_path(NULL),
      ...
      image_file(image_file_in)
    {
      int width = 800;
      int height = 800;

      device = irr::createDevice(
        irr::video::EDT_OPENGL, irr::core::dimension2d < irr::u32 > (width, height), 32, false,
        false,
        false);
      ...
      proj.buildProjectionMatrixOrthoLH(
        (irr::f32)width, (irr::f32)height, (irr::f32)0.01, (irr::f32)100);
      ...
      if (irat_texture) {
        ...
        irat_node = exp_map_scene->addMeshSceneNode(
          exp_map_scene->getGeometryCreator()->createCubeMesh(
            irr::core::vector3df((irr::f32)(535.0 / 8.0), (irr::f32)(289.0 / 8.0), (irr::f32)0.1)),
          NULL);
      }
    ...
    void update_links()
    {
      ...
      for (unsigned int i = 0; i < map->links.size(); i++) {
        x2d1 = (int)((
            (map->experiences[map->links[i].exp_from_id].x_m - min_x) / (max_x - min_x) *
            viewport_width));
        y2d1 = (int)((
            (map->experiences[map->links[i].exp_from_id].y_m - min_y) / (max_y - min_y) *
            viewport_height));
        x2d2 = (int)((
            (map->experiences[map->links[i].exp_to_id].x_m - min_x) / (max_x - min_x) *
            viewport_width));
        y2d2 = (int)((
            (map->experiences[map->links[i].exp_to_id].y_m - min_y) / (max_y - min_y) *
            viewport_height));
    ...
    void update_goal_list(const std::deque < int > & goals)
    {
      ...
      for (i = 0; i < goals.size(); i++) {
        ...
        numberNodes[i] = exp_map_scene->addBillboardTextSceneNode(
          ...
          irr::core::vector3df(
            (irr::f32)((map->experiences[goals[i]].x_m - min_x) / (max_x - min_x) * viewport_width),
            (irr::f32)((map->experiences[goals[i]].y_m - min_y) / (max_y - min_y) *
            viewport_height),
            (irr::f32)0.0),
          -1, -1);
      }
      if (map->waypoint_exp_id != -1) {
        ...
        numberNodes[i] = exp_map_scene->addBillboardTextSceneNode(
          ...
          irr::core::vector3df(
            (irr::f32)(
              (map->experiences[map->waypoint_exp_id].x_m - min_x) / (max_x - min_x) *
              viewport_width),
            (irr::f32)(
              (map->experiences[map->waypoint_exp_id].y_m - min_y) / (max_y - min_y) *
              viewport_height),
            (irr::f32)0.0),
          -1, -1);
      }
    ...
    void update_viewport(int x, int y, int width, int height)
    {
      ...
      irr::scene::ICameraSceneNode * exp_map_camera = exp_map_scene->addCameraSceneNode(
        NULL,
        irr::core::vector3df(
          (irr::f32)(0.5 * viewport_width), (irr::f32)(0.5 * viewport_height), (irr::f32)-40),
        irr::core::vector3df(
          (irr::f32)(0.5 * viewport_width), (irr::f32)(0.5 * viewport_height), (irr::f32)0));
      irr::core::matrix4 proj;
      proj.buildProjectionMatrixOrthoLH(
        (irr::f32)(1.0 * viewport_width), (irr::f32)(1.0 * viewport_height), (irr::f32)0.01,
        (irr::f32)100);
